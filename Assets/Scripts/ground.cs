   ...
   void OnCollisionEnter2D(Collision2D col){
    ...
    void ReSpawn(){
		...
		for(int count=0 ; count < maxBlock; count++){
			...
			RandomPosisi(posArr);
			GameObject newBrick =Instantiate(bricks, new Vector3(UKURAN_BALOK*(posArr[count])-2.42f, 3.5f-UKURAN_BALOK, 0), Quaternion.identity);
		...
		bricksRow.Add(Instantiate(AddBall, new Vector3(UKURAN_BALOK*addBallPos-2.42f, 3.5f-UKURAN_BALOK, 0), Quaternion.identity));
