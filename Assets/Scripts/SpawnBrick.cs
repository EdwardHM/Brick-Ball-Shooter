...
public class SpawnBrick : MonoBehaviour
{
    ...
    void Start()
    {
		...
		for(int count=0 ; count < maxBlock; count++){
			...
			RandomPosisi(posArr);
			bricksRow.Add(Instantiate(bricks, new Vector3(f*(posArr[count])-2.42f, 3.5f-f, 0), Quaternion.identity));
		}
		int addBallPos = RandomPowerUp(posArr);
		bricksRow.Add(Instantiate(AddBall, new Vector3(f*addBallPos-2.42f, 3.5f-f, 0), Quaternion.identity));
